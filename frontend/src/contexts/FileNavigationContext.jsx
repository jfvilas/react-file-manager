...
export const FileNavigationProvider = ({ children, initialPath, onFolderChange }) => {
    ...
    const [currentOwnLayoutPath, setCurrentOwnLayoutPath] = useState(null)
    ...
    useEffect(() => {
        if (Array.isArray(files) && files.length > 0) {
            ...
            let file = files.find(x => x.path===currentPath)
            if (file && file.layout==='own') {
                setCurrentOwnLayoutPath(currentPath)
            }
            else {
                setCurrentOwnLayoutPath(undefined)
            }
        }
        else {
            setCurrentPathFiles([])
    ...
    return (
        <FileNavigationContext.Provider
            value={{
                currentOwnLayoutPath,
                setCurrentOwnLayoutPath,
                currentPath,
                setCurrentPath,
                currentFolder,
                setCurrentFolder,
