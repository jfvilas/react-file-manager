...
export const useShortcutHandler = (triggerAction, onRefresh, permissions) => {
    ...
    const triggerSearch = () => {
        if (document.getElementById('jfvilas-react-file-manager-search-id')) document.getElementById('jfvilas-react-file-manager-search-id').focus()
    }

    const triggerDelete = () => {
        if (permissions.delete && selectedFiles.length) triggerAction.show("delete")
    }
    ...
    const triggerSelectFirst = () => {
        if (currentPathFiles.length > 0) setSelectedFiles([currentPathFiles[0]])
    }

    const triggerSelectLast = () => {
      if (currentPathFiles.length > 0) setSelectedFiles([currentPathFiles.at(-1)])
    }
