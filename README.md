...
type File = {
  ...
  class?: string; // Optional: a name for selecting specific icons
};
```
...
| `icons` |   Map<string, { open?: JSXElement, closed? :JSXElement>  | A Map containing specific icons for showing according to file 'class' attribute. Each file entry in the file list can include a specific `class` attribute that holds a file class name. This name is the key to the icons Map, so when the component renders a file, the icon used on the render process will be a default icon (folder, file,...) or a specific icon provided by this Map via a JSX element. You can provide 2 icons (one for open state and another one for closed state) |
